  ...
  return (
    <section className="space-y-golden-md">
      ...
      <div className="grid gap-golden-base sm:grid-cols-2">
        {userPosts.map((post) => (
          <Card key={post.id} className="p-golden-base flex flex-col h-full group hover:-translate-y-1 transition-transform duration-300 will-change-transform">
            ...
            <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 flex justify-end">
               <span className="text-xs font-bold text-blue-600 dark:text-blue-400 opacity-0 group-hover:opacity-100 transition-opacity duration-300 cursor-pointer hover:underline">
                Leer más →
              </span>
            </div>
          </Card>
...
PostListComponent.propTypes = {
  userPosts: PropTypes.arrayOf(PropTypes.shape(zodToPropTypes(PostSchema))).isRequired,
};
