...
const ErrorMessage = memo(({ message, onRetry }) => {
  ...
  const displayMessage = message === "error.generic" 
  ...
  return (
    <div className={cn(
      ...
      <div className={cn("w-16 h-16 bg-red-100 dark:bg-red-900/30 rounded-full flex items-center justify-center mx-auto mb-4")}>
        <span className={cn("text-3xl text-red-600")}>⚠️</span>
      </div>
      <h3 className={cn("text-xl font-bold text-red-900 dark:text-red-400 mb-2")}>
        ¡Ups! Algo salió mal
      </h3>
      <p className={cn("text-red-700 dark:text-red-500/80 mb-6 font-medium")}>
        {displayMessage}
      </p>
      {onRetry && (
        <button
